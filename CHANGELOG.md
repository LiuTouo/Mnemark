...
- 防止重複啟動：第二次啟動時新行程於外掛初始化階段（熱鍵、常駐圖示與剪貼簿監聽開始之前）即退出，改由已在執行的實例顯示面板，效果等同按下熱鍵；以 `--hidden` 啟動（自動啟動）時仍保持安靜不彈出。鎖定以應用程式識別碼為鍵，安裝版與攜帶版同時執行也視為同一實例
...
- 徹底消除開啟歷史工作區時瞬間閃現的原生標題列：tao 每次顯示都重寫樣式並觸發框架變更，使 DWM 先鎖住了帶標題列的框架；現於清除標題列位元後立即再發一次不改變幾何的框架變更，讓 DWM 重新以乾淨樣式鎖框，並同時停用此視窗的非客戶區描繪，雙重保證不再閃現

## [0.8.7] - 2026-09-22

### Fixed

- 開啟歷史工作區時不再瞬間閃現原生標題列與右上角傳統的最小化／最大化／關閉按鈕：tao 在無邊框視窗的樣式中仍保留標題列位元並於每次顯示時重寫，現於每次顯示後立即清除，DWM 無從描繪
...
- 主視窗改為固定大小的透明宿主，一次預留歷史、抽屜與預覽的完整範圍：開關側邊面板不再改變原生視窗尺寸，改以 Windows 視窗區域控制繪圖與輸入範圍——展開前先擴大區域、畫面繪製完成後才釋放閒置空間，消除開關面板瞬間的畫面撕裂與殘影；歷史面板於寬度足夠時保持置中

### Added

- 網站加入 favicon，沿用主程式圖示
...
- 修正多選較舊的歷史項目時清單跳回頂端：勾選、取消勾選及全選會保留原捲動位置，方向鍵導航仍會捲至選取項目
## [0.8.2] - 2026-09-12

### Added

- 系統列圖示現在會顯示監聽狀態：暫停監聽時圖示左下角疊上暫停角標，繼續監聽時恢復原圖示

### Changed

- 剪貼簿歷史持久化改為預設開啟：新安裝的歷史紀錄會跨重啟保留；需要在結束時清除歷史者可於設定關閉持久化（既有已明確設定的使用者不受影響）
...
- 修正間歇性「Ctrl+C 複製後 Ctrl+V 貼上沒有東西」：Office 等應用程式以延遲渲染提供剪貼簿資料，擷取過程強制其現場渲染，此時若有第三方（雲端剪貼簿服務、其他剪貼簿工具）開啟剪貼簿，渲染結果會丟失，後續貼上即為空
- 擷取改為每次剪貼簿變更只以免開剪貼簿的格式探測選出單一格式、並在單一 session 內讀取（原為三種格式逐一開啟探測，最多強制三次渲染，每次都是新的丟失機會）
- 渲染讀取失敗時改記「延遲項目」（deferred Clip，含來源與擷取時的剪貼簿序號），不再重試逼迫渲染；同一來源視窗的重複複製會合併為一筆
- 貼上延遲項目時，若剪貼簿序號未變（內容仍在）則跳過寫入、由貼上目標自行向來源應用程式取得內容，完整保留 Office 的原生格式；剪貼簿已變更則明確回報「內容已無法擷取」，不再貼出錯誤內容

## [0.7.9] - 2026-09-02

### Changed

- 面板歷史紀錄改由單一模組管理：擷取合併、刪除、批次復原與釘選皆走同一介面，移除重複的復原收尾流程
- 面板焦點喪失收合、貼上時序與預覽 generation 三條時序邏輯集中於注入時脈與前景視窗來源的獨立模組，並以虛擬時脈全面測試
- Esc 分層收合（選單→對話框→拖曳）由單一擁有者判定，移除跨模組的 DOM 探刮與合成點擊，每次按鍵至多取消一次拖曳
- 抽屜全部七個 mutation 站點（含移除與收藏集新增／重新命名／刪除／排序）統一經過共用的 commit-then-refresh 工作流程，失敗復原語意不再分歧
- Clip 定位解析收斂至單一介面；加入收藏改於 aggregate 鎖內解析，移除鎖外解析窗口
- 設定載入契約與啟動程序（語系、主題、透明度夾限）由單一設定模組負責，i18n 回歸純字典
- 面板開啟後選取項目時，若 1 秒內未能確認原視窗，改為不貼上、內容保留於剪貼簿供手動貼上（原為貼入當時前景視窗，可能貼錯）
- Esc 在拖曳中且選單或對話框開啟時，先收合選單與對話框、再取消拖曳（原為先取消拖曳）

### Fixed

- 修正設定視窗與其他視窗的透明度夾限不一致；異常值（如 NaN）現在統一回到預設 99
