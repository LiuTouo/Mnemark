...
export function renderPage(locale: Locale, base: string, release: Release) {
  ...
  <section id="overview" class="overview wrap section-space" aria-labelledby="overview-title"><div class="section-label"><span>01 / ${c.nav[0]}</span>${icon("copy")}</div><div class="overview-grid"><h2 id="overview-title">${lines(c.introTitle)}</h2><div><p class="large-copy">${c.introText}</p><p class="muted">${c.introNote}</p></div></div><div class="quick-flow">${c.quick.map((step, i) => `<div><span class="quick-num">0${i + 1}</span>${icon(["copy", "grid", "arrow"][i])}<h3>${step}</h3><kbd>${["Ctrl + C", "Ctrl + Shift + V", locale === "zh" ? "Enter 或點擊" : "Enter or click"][i]}</kbd></div>`).join("")}</div><div class="quick-extra"><p>${c.optionalSearch}</p><a href="#feature-search">${c.optionalSearchLink} ↗</a></div></section>
