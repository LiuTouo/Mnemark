...
獨立的繁中／英文靜態網站，使用 Vite、TypeScript、GSAP 與 Lenis 平滑捲動。網站不呼叫 Tauri、不讀取訪客剪貼簿，也沒有帳號、後端或追蹤服務。畫面中的產品操作使用示範資料。
...
官網功能依目前 v0.8.1 專案原始碼核對。之後修改產品時，應同步檢查文案、操作示意與以下界線：搜尋不是全文／OCR／語意搜尋；檔案是路徑參照；抽屜保存獨立於歷史持久化；釘選是否跨重啟保存取決於歷史持久化設定。
